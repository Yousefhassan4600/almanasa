    ...
    <div style="display: grid; gap: 0.75rem;">
        @forelse($answers as $answer)
            @php
                ...
                $scoreLabel = $answer->score !== null ? number_format((float) $answer->score, 2) : ($requiresManualGrading ? __('admin.labels.Pending') : '-');
            @endphp
            ...
            <div style="border: 1px solid #e5e7eb; border-radius: 10px; padding: 1rem; background: #ffffff;">
                ...
                <div style="display: grid; gap: 0.4rem; color: #374151; font-size: 0.85rem;">
                    ...
                    @endif
                    <div>
                        <span style="font-weight: 700;">{{ __('admin.labels.Score') }}:</span>
                        <span>
                            {{ $scoreLabel }}
                            /
        ...
        @empty
            <div style="border: 1px dashed #cbd5e1; border-radius: 10px; padding: 1rem; color: #64748b; text-align: center;">
                {{ __('admin.messages.no_answers_found_for_this_attempt') }}
            </div>
        @endforelse
    </div>
</div>
