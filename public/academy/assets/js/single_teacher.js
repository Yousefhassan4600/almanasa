function switchMainTab(tabId) {
  ...
  if (tabId === "lessons") {
    lessonsBtn.className =
      "pb-3 text-sm font-bold border-b-2 border-[#5D3FD3] text-[#5D3FD3] transition-all";
...
function filterTerm(term) {
  ...
  document.getElementById(`btn-sub-${term}`).className =
    "term-btn px-4 py-1.5 text-xs font-bold rounded-lg bg-[#5D3FD3] text-white transition-all shadow-sm";
...
function filterStudentTerm(term) {
  ...
  if (activeBtn) {
    activeBtn.className =
      "student-term-btn px-4 py-1.5 text-xs font-bold rounded-lg bg-[#5D3FD3] text-white transition-all shadow-sm";
  }
  ...
  // Update DOM elements
  document.getElementById("student-general-level").textContent = data.generalLevel;
  document.getElementById("student-general-level-percent").textContent = data.generalLevelPercent;
  document.getElementById("student-time-spent").textContent = data.timeSpent;
  document.getElementById("student-lessons-ratio").textContent = data.attendanceLessons;
  document.getElementById("student-homework-correct").textContent = data.homeworkCorrect;
  document.getElementById("student-homework-wrong").textContent = data.homeworkWrong;
  document.getElementById("student-homework-rate").textContent = data.homeworkRate;
  document.getElementById("student-exams-avg").textContent = data.examsAvg;

  // Animate linear progress bar
  const progressBar = document.getElementById("student-attendance-progress-bar");
  if (progressBar) {
  ...
  const attendancePercentText = document.getElementById("student-attendance-percent");
  if (circleProgress) {
    ...
    if (data.exams.length === 0) {
      ...
      examsList.innerHTML = data.exams.map(exam => `
        <div class="flex items-center justify-between p-4 bg-[#F8F7FF] rounded-2xl border border-[#ECECFF]/50 hover:border-[#5D3FD3]/25 transition-all duration-300 shadow-sm hover:shadow-md">
             ...
             <div class="text-left">
                 <span class="text-xs font-bold text-[#5D3FD3] block">${exam.score}</span>
  ...
  const opportunitiesList = document.getElementById("student-opportunities-list");
  if (opportunitiesList) {
    opportunitiesList.innerHTML = data.opportunities.map(opportunity => `
      <li class="flex items-center gap-2">
